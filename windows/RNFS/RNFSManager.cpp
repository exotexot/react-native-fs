...
CancellationDisposable::CancellationDisposable(IAsyncInfo const &async, std::function<void()> &&onCancel) noexcept
    : m_async{async}, m_onCancel{std::move(onCancel)}
{
}

CancellationDisposable::CancellationDisposable(CancellationDisposable &&other) noexcept
    : m_async{std::move(other.m_async)}, m_onCancel{std::move(other.m_onCancel)}
{
}
...
//void RNFSManager::exists3(std::string filepath, RN::ReactPromise<bool> promise) noexcept
//try
//{
//    std::filesystem::path file{srcFolderPath};
//    file.make_preferred();
//
//    StorageFile file{co_await StorageFolder::GetFileAsync(winrt::to_hstring(file.c_str()))};
//
//    if (file.IsOfType(StorageItemTypes::None))
//    {
//        promise.Reject();
//    }
//    else
...
winrt::fire_and_forget RNFSManager::exists2(std::string filepath, RN::ReactPromise<bool> promise) noexcept
try
{
    winrt::hstring directoryPath, fileName;
    splitPath(filepath, directoryPath, fileName);
